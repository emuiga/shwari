...
import { recentLeads } from '@/features/provider/dashboard/presentation/lib/mockDashboardData';
...
export default function ProviderDashboardPage({ reviews }: ProviderDashboardPageProps) {
  const { listings } = useServiceListings();
  const unreadMessages = 2;
  const averageRating =
    reviews.length > 0 ? (reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length).toFixed(1) : '—';
  ...
  return (
    <div className="min-h-screen w-full bg-surface-muted lg:bg-transparent">
      <ProviderHeader />
      <main className="px-4 py-6 sm:px-6">
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <section className="rounded-card border border-border-soft bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-subtle">Recent Leads</h2>
              <Link href="/leads" className="text-xs font-semibold text-primary-strong hover:underline">
                View all
              </Link>
            </div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span
                        className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                          lead.status === 'new' ? 'bg-primary-subtle text-primary-strong' : 'bg-surface-muted text-subtle'
                        }`}
                      >
                        {lead.status === 'new' ? 'New' : 'Contacted'}
                      </span>
                      <span className="text-[11px] text-faint">{formatRelativeDate(lead.requestedAt)}</span>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <section className="rounded-card border border-border-soft bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-wide text-subtle">Recent Reviews</h2>
              <Link href="/company-profile?tab=reviews" className="text-xs font-semibold text-primary-strong hover:underline">
                View all
              </Link>
            </div>
              ...
              <ul className="mt-4 divide-y divide-gray-100">
                {reviews.slice(0, 3).map((review) => (
                  <li key={review.id} className="py-3">
                    ...
                    {review.comment && <p className="mt-1 text-xs text-subtle">{review.comment}</p>}
                    {(review.serviceName || review.createdAt) && (
                      <p className="mt-1 text-[11px] text-faint">
                        {[review.serviceName, review.createdAt].filter(Boolean).join(' · ')}
                      </p>
                    )}
                  </li>
                ))}
        ...
        <section className="mt-8">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-subtle">Service Listings</h2>
          ...
          {listings.length === 0 ? (
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {listings.slice(0, 3).map((listing) => (
                <div key={listing.id} className="rounded-card border border-border-soft bg-white p-4 shadow-sm">
                  <p className="text-sm font-semibold text-ink">{listing.categoryName ?? 'Service'}</p>
                  <p className="mt-1 text-xs text-subtle">{(listing.serviceAreas ?? []).slice(0, 2).join(', ')}</p>
                </div>
              ))}
            </div>
          )}
